var lunr=function(config){var idx=new lunr.Index
...
lunr.utils.zeroFillArray=(function(){var zeros=[0]
return function(length){while(zeros.length<length){zeros=zeros.concat(zeros)}
...
break}}
return str.split(/\s+/).map(function(token){return token.replace(/^\W+/,'').replace(/\W+$/,'').toLowerCase()})}/*!
* lunr.Pipeline
* Copyright (C) 2013 Oliver Nightingale
*/
...
* Copyright (C) 2013 Oliver Nightingale
*/
lunr.Vector=function(elements){this.elements=elements}
...
for(var i=0;i<len;i++){el=elems[i]
sumOfSquares+=el*el};return this._magnitude=Math.sqrt(sumOfSquares)}
lunr.Vector.prototype.dot=function(otherVector){var elem1=this.elements,elem2=otherVector.elements,length=elem1.length,dotProduct=0
for(var i=0;i<length;i++){dotProduct+=elem1[i]*elem2[i]};return dotProduct}
...
lunr.Index.prototype.idf=function(term){if(this._idfCache[term])return this._idfCache[term]
...
lunr.Index.prototype.search=function(query){var queryTokens=this.pipeline.run(lunr.tokenizer(query)),queryArr=lunr.utils.zeroFillArray(this.corpusTokens.length),documentSets=[],fieldBoosts=this._fields.reduce(function(memo,f){return memo+f.boost},0)
...
lunr.Index.prototype.documentVector=function(documentRef){var documentTokens=this.documentStore.get(documentRef),documentTokensLength=documentTokens.length,documentArr=lunr.utils.zeroFillArray(this.corpusTokens.length)
...
documentArr[this.corpusTokens.indexOf(token)]=tf*idf};return new lunr.Vector(documentArr)}
...
return memo},{})
return store}
lunr.Store.prototype.set=function(id,tokens){this.store[id]=tokens
...
return memo}
lunr.TokenStore.prototype.toJSON=function(){return{root:this.root,length:this.length}}